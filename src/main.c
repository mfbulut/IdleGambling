...
char* text[] = {
    ...
    "Misket", "Ball",
    "Hız", "Speed",
    "Katlayıcı", "Multiplier",
    "Hepsi Satıldı", "Sold Out",
    "Kart Çek", "Hit",
    "Kal", "Stand",
    "Bahis: %s", "Bet: %s",
    "Dağıtıcı", "Dealer",
    ...
    "Seviye Atla", "Level Up",
    "Sigorta", "Insurance",
    "Kaybın %10'u geri alınır", "Get %10 payback on lose",
    "Hızlı Eller", "Fast Hand",
    "Dağıtıcıyla elini değiştir", "Swap hands with dealer",
    "Hilekar", "The Cheater",
    "Dağıtıcının kartlarını gör", "See dealers hand",
    "İptal", "Cancel",
    "KAZANDIN", "WIN",
    "KAYBETTİN","LOSE",
    "BERABERE", "DRAW",
    "Altın Top", "Golden Ball",
    "Bir top 2X para kazandırır", "One ball gives 2X money",
    "Devam et", "Continue",
    "Bonus", "Bonus",
    "Seç", "Equip",
    "Seçili", "Equipped"
...
Cosmetic cosmetics[MAX_COSMETICS] = {
    ...
    {"Kartal Desenli", "Eagle Pattern", 30000, 2},
    {"Boğa Desenli", "Bull Pattern",  500000, 4},
    {"Aslan Desenli", "Lion Pattern", 1000000, 4},
};
    ...
    BeginDrawing();
        ...
        switch(currentScreen)
        {
            ...
            case SETTINGS:
            {
                ...
                #if defined(PLATFORM_WEB) || defined(_WIN32) || defined(__linux__)
                if(Red_Button((Rectangle){ 10, 20, 150, 50 }, "Bonus", (Color){ 143, 52, 30, 255 })) {
                    bonusTimer = 6 * 60;
                }

                if(Red_Button((Rectangle){ 10, 80, 150, 50 }, "Money", (Color){ 143, 52, 30, 255 })) {
                    money += 1000000;
                    SaveState();
                }
                #endif

                // if(Red_Button((Rectangle){ 10, 190, 150, 50 }, "Show Ad", (Color){ 143, 52, 30, 255 })) {
                //     #ifndef _WIN32
                //     showAd = true
                //     RequestInterstitialAd();

                //     if(showAd && IsInterstitialAdLoaded()){
                //        ShowInterstitialAd();
                //     }
                //     #endif
                // }

                DrawTexture(trFlag, 20, 460, WHITE);
                DrawTexture(enFlag, 20 + 64 + 20, 460, WHITE);
